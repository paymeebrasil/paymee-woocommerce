 ...
 *
 * @package Woo_PayMee/Classes/API
 * @version 1.0.0
 */
 ...
 */
class WC_Woo_PayMee_Pix_API {

	/**
	 * Gateway class.
	 *
	 * @var WC_Woo_PayMee_Pix_Gateway
	 */
	protected $gateway;
	 ...
	 * Constructor.
	 *
	 * @param WC_Woo_PayMee_Pix_Gateway $gateway Payment Gateway instance.
	 */
	...
	protected function get_checkout_url() {
		return 'https://' . $this->get_environment() . 'paymee.com.br/v1.1/checkout/transparent';
	}
	...
	protected function get_payment_url( $token ) {
		return 'https://' . $this->get_environment() . '.paymee.com.br/redir/' . $token;
	}
	...
	public function get_payment_name_by_type( $value ) {
		$types = array(
			1 => __( 'Bank Transfer', 'woocommerce-paymee-pix' ),
			2 => __( 'Cash Payment', 'woocommerce-paymee-pix' ),
		);

		return isset( $types[ $value ] ) ? $types[ $value ] : __( 'Unknown', 'woocommerce-paymee-pix' );
	}
	...
	public function get_payment_method_name( $value ) {
		$transfer = __( 'Bank Transfer %s', 'woocommerce-paymee-pix' );
		$cash = __( 'Cash Payment %s', 'woocommerce-paymee-pix' );
		...
		return isset( $methods[ $value ] ) ? $methods[ $value ] : __( 'Unknown', 'woocommerce-paymee-pix' );
	}
	...
	public function get_error_message( $code ) {
		...
		$messages = array(
			'-1' => __( 'Falha em validar as informações fornecidas, verifique o erro no log e tente novamente.', 'woocommerce-paymee-pix' ),
			'998' => __( 'Não foi possivel recuperar a transação pelo identificador informado.', 'woocommerce-paymee-pix' ),
			'999' => __( 'A situação da transação não está pendente.', 'woocommerce-paymee-pix' ),
			'1000' => __( 'A transação não está com o status Pago ou não existe.', 'woocommerce-paymee-pix' ),
			'1001' => __( 'O código de referência informado já existe para outra venda.', 'woocommerce-paymee-pix' ),
		);
		...
		return __( 'Ocorreu um erro, tente novamente ou contate o administrador do site.', 'woocommerce-paymee-pix' );
	}
	...
	protected function get_order_items( $order ) {
		...
		if ( 'yes' == $this->gateway->send_only_total ) {
			$items[] = array(
				'description' => $this->sanitize_description( sprintf( __( 'Order %s', 'woocommerce-paymee-pix' ), $order->get_order_number() ) ),
	...
	public function do_checkout_request( $order, $posted ) {
		...
		$json = array(
			...
			"maxAge" => 1440,
			"paymentMethod" => 'PIX',
			"callbackURL" =>  site_url() . "/?wc-api=paymee_pix_ipn_listener",
			"shopper" => array(
		...
		// 66355042-224a-344d-bfe0-9afd23006b94
		// 863bbc2f-f120-365f-b102-b7b63ef0e5e6
		...
		update_post_meta( $order->get_id(), 'paymee_pix_info', json_decode($response['body']));
		...
		if ( is_wp_error( $response ) ) {
			...
			return array(
				'url'   => '',
				'data'  => '',
				'error' => array( __( 'Falha em suas credenciais da PayMee do Brasil!', 'woocommerce-paymee-pix' ) ),
			);
		} else {
			try {
			...
			if ( isset( $body->error ) ) {
				...
				foreach ( $body->error as $error_key => $error ) {
					if ( $message = $this->get_error_message( $error->code ) ) {
						$errors[] = '<strong>' . __( 'PayMee', 'woocommerce-paymee-pix' ) . '</strong>: ' . $message;
					}
				}
		...
		return array(
			'url'   => '',
			'token' => '',
			'error' => array( '<strong>' . __( 'PayMee', 'woocommerce-paymee' ) . '</strong>: ' . __( 'Ocorreu um erro, tente novamente ou contate o administrador do site.', 'woocommerce-paymee-pix' ) ),
		);
	}
}
